...
" excscope#set_gtagsfile {{{2
function excscope#set_gtagsfile(path)
    let s:gtagsfile = a:path
endfunction
...
" excscope#connect_gtags {{{2
function excscope#connect_gtags()
    " don't show any message
	setlocal nocsverb
    " connect gtags files
    set csprg=gtags-cscope
    silent exec "cscope add " . s:gtagsfile
	silent! setlocal cscopequickfix=s-,c-,d-,i-,t-,e-
endfunction 
...
function excscope#kill_gtags()
    " kill gtags files
    silent exec " cscope kill " . fnamemodify(s:gtagsfile, ':p')
endfunction 
