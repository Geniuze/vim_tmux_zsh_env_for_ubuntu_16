...
function exconfig#apply()
    ...
    " set gtags file path
    if vimentry#check('enable_gtags', "true")
"        call excscope#set_gtagsfile(g:exvim_folder.'/gtags/GTAGS')
        call exconfig#gen_sh_update_gtags(g:exvim_folder)
"        call excscope#connect_gtags()
    endif
...
" gtags -f file _gtags
" if [ -d "./_gtags" ]; then
" echo "  |- move _gtags to ./${vimfiles_path}/gtags"
" mv -f "_gtags" "./${vimfiles_path}/gtags"
" fi
" echo "  |- done!"
" --------------------------------------------------
...
function exconfig#gen_sh_update_gtags(path)
    ...
    if ex#os#is('windows')
        ...
        let scripts = [
                    ...
                    \ 'export TMP="${DEST}/_gtags"'           ,
                    \ 'export TARGET="${DEST}/gtags"'         ,
